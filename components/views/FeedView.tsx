  ...
  const publish = async () => {
    ...
    try {
      if (newImg) {
        ...
        const { data: up } = await supabase.storage.from("messages").upload(fn, cf);
        if (up) mediaUrl = supabase.storage.from("messages").getPublicUrl(up.path).publicUrl;
      }

      if (voiceDraft) {
        const fn = `${me}/voice/${Date.now()}.webm`;
        const { data: up } = await supabase.storage.from("messages").upload(fn, voiceDraft);
        if (up) voiceUrl = supabase.storage.from("messages").getPublicUrl(up.path).publicUrl;
      }
              ...
              <button
                ...
                onClick={() => {
                  if (bookmarks.includes(p.id)) {
                    ...
                    // Temporarily store post id for collection
