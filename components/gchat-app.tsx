...
export function GChatApp() {
  ...
  useEffect(() => {
    ...
    if (view === "profile" || view === "edit-profile" || view === "analytics") {
      fetchProfile();
    }
  }, [user, view]);
  ...
  const claimReward = async (adId: string) => {
    try {
      const { error } = await supabase.rpc('claim_ad_reward', { target_campaign_id: adId });      if (error) throw error;
  ...
  return (
    <div className="relative min-h-screen w-full max-w-md mx-auto text-white overflow-hidden font-sans">
      ...
      {view === "conversation" && activeChatId && (
        <ConversationView          setView={setView}
      ...
      {view === "edit-profile" && (
        <EditProfileView
          ...
          onCoverChange={(e) => { 
            ...
            if(f) { 
              setProfileCover(f); 
              setProfileCoverPreview(URL.createObjectURL(f)); 
            }
          }}
      ...
      {/* Modals */}
      {showNewChat && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setShowNewChat(false)}>          <div className="w-full max-w-sm rounded-2xl bg-[#0B1120] border border-white/10 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold">New Chat</h3>
              <button onClick={() => setShowNewChat(false)} className="p-2 rounded-full hover:bg-white/5">
                <X className="h-5 w-5" />
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setShowWithdraw(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-[#0B1120] border border-white/10 p-6" onClick={e => e.stopPropagation()}>
            ...
            <input 
              ...
              className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white mb-3" 
            />
            <input 
              value={bankName} 
            ...
            <input 
              ...
              className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white mb-4" 
            />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setShowBusinessSetup(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-[#0B1120] border border-white/10 p-6" onClick={e => e.stopPropagation()}>
            ...
            <textarea 
              ...
              placeholder="AI Auto-Reply message" 
              className="w-full h-20 rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white mb-4 resize-none" 
            />
            <div className="flex gap-2">
              <button onClick={() => setShowBusinessSetup(false)} className="flex-1 py-3 rounded-xl bg-white/5">
                Cancel
              </button>
              <button onClick={handleSetupBusiness} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 font-bold">
                Setup
              </button>
            </div>
          </div>
      ...
      {showCreateGroup && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => setShowCreateGroup(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-[#0B1120] border border-white/10 p-6" onClick={e => e.stopPropagation()}>
            ...
            />
            <div className="flex gap-2">
              <button onClick={() => setShowCreateGroup(false)} className="flex-1 py-3 rounded-xl bg-white/5">
                Cancel
              </button>
              <button onClick={handleCreateGroup} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 font-bold">
                Create
              </button>
            </div>
          </div>        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4" onClick={() => !isPosting && setShowCreatePost(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-[#0B1120] border border-white/10 p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              ...
              <button onClick={() => !isPosting && setShowCreatePost(false)} className="p-2 rounded-full hover:bg-white/5">
                <X className="h-5 w-5" />
              </button>
            </div>
            <textarea 
              ...
              className="w-full h-24 rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white outline-none mb-4 resize-none" 
            />
            {postImagePreview && (
              <img src={postImagePreview} alt="Preview" className="w-full h-48 object-cover rounded-xl mb-4" />
            )}
            <div className="flex gap-2">
              <button 
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4" onClick={() => setShowSendMoney(false)}>
          <div className="w-full max-w-sm rounded-2xl bg-[#0B1120] border border-white/10 p-6" onClick={e => e.stopPropagation()}>
            ...
            <input 
              ...
              placeholder="Amount (USD)" 
              className="w-full rounded-xl bg-white/5 border border-white/10 px-4 py-3 text-white mb-4 outline-none" 
            />
            <div className="flex gap-2">
              <button onClick={() => setShowSendMoney(false)} className="flex-1 py-3 rounded-xl bg-white/5">
                Cancel
              </button>
              <button onClick={handleSendMoney} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-cyan-500 font-bold">
                Send
              </button>
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4">
          <div className="w-full max-w-sm rounded-2xl bg-[#0B1120] border border-white/10 p-8 text-center">
            ...
            <p className="text-xs text-gray-500">
              Watching... {Math.floor(adProgress / 10)}s / 3s
            </p>
            {adProgress === 100 && (
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-40 bg-[#020617]/95 backdrop-blur-xl border-t border-white/10 max-w-md mx-auto pb-safe">
        <div className="flex justify-around items-center h-16">
          <button 
            ...
            className={`flex flex-col items-center gap-1 ${view === "home" ? "text-cyan-400" : "text-gray-500"}`}
          >
          ...
          <button 
            ...
            className={`flex flex-col items-center gap-1 ${view === "list" || view === "conversation" ? "text-cyan-400" : "text-gray-500"}`}
          >
          ...
          >
            ...
            <span className="text-[9px]">G-Pay</span>
          </button>
          <button 
            onClick={() => setView("profile")} 
            className={`flex flex-col items-center gap-1 ${view === "profile" || view === "edit-profile" || view === "analytics" || view === "settings" ? "text-cyan-400" : "text-gray-500"}`}
          >
